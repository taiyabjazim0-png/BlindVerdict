...
export function ClientReviewPage() {
  ...
  const reviewMutation = useMutation({
    mutationFn: () => platformService.submitReview({ case_id: selectedCaseId, target_user_id: currentCase?.client_id || 0, score, feedback }),
  ...
  return (
    <div className="max-w-2xl space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-slate-900">Client Review</h1>
        <p className="mt-1 text-sm text-slate-500">Rate your clients. Visible only to administrators for flagging.</p>
      </div>
      ...
      {selectedCaseId > 0 && currentCase && (
        <div className="rounded-2xl border border-slate-200/60 bg-white p-6 shadow-sm">
          <h3 className="font-bold text-slate-900">Review: {currentCase.client_name || "Client"}</h3>
